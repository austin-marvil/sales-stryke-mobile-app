"use strict";
/**
 * This file is auto generated DO NOT EDIT.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.ImportDataMap = void 0;
const ClassFor_1 = require("../../../data/ClassFor");
const JsonApiPersistableBase_1 = require("../../../jsonApi/JsonApiPersistableBase");
const HashCodeUtil_1 = require("../../../utils/HashCodeUtil");
const PersistenceType_1 = require("../../../data/PersistenceType");
const PersistableUtils_1 = require("../../../data/PersistableUtils");
class EntityAttributes {
    constructor() {
        this._allowImportDataToBeUpdatedIsMutated = "_allowImportDataToBeUpdatedIsMutated";
        this._dataMapTokenIsMutated = "_dataMapTokenIsMutated";
        this._descriptionIsMutated = "_descriptionIsMutated";
        this._entityState = "_entityState";
        this._idSetBySystem = "_idSetBySystem";
        this._importDataMapAttributesIsMutated = "_importDataMapAttributesIsMutated";
        this._importDataMapTypeIsMutated = "_importDataMapTypeIsMutated";
        this._initializedDataKey = "_initializedDataKey";
        this._initializedDataKeyIsMutated = "_initializedDataKeyIsMutated";
        this._isAddToRelationship = "_isAddToRelationship";
        this._isAddToRelationshipIsMutated = "_isAddToRelationshipIsMutated";
        this._jsonStateStr = "_jsonStateStr";
        this._nameIsMutated = "_nameIsMutated";
        this._organizationIsMutated = "_organizationIsMutated";
        this._relationshipPersistableType = "_relationshipPersistableType";
        this.allowImportDataToBeUpdated = "allowImportDataToBeUpdated";
        this.createdDate = "createdDate";
        this.dataMapToken = "dataMapToken";
        this.description = "description";
        this.id = "id";
        this.importDataMapAttributes = "importDataMapAttributes";
        this.importDataMapType = "importDataMapType";
        this.jsonType = "jsonType";
        this.modifiedDate = "modifiedDate";
        this.name = "name";
        this.organization = "organization";
    }
}
class MongoVirtual {
}
class MongoAttributes {
    constructor() {
        this.virtual = new MongoVirtual();
        this._allowImportDataToBeUpdatedIsMutated = "_allowImportDataToBeUpdatedIsMutated";
        this._dataMapTokenIsMutated = "_dataMapTokenIsMutated";
        this._descriptionIsMutated = "_descriptionIsMutated";
        this._entityState = "_entityState";
        this._idSetBySystem = "_idSetBySystem";
        this._importDataMapAttributesIsMutated = "_importDataMapAttributesIsMutated";
        this._importDataMapTypeIsMutated = "_importDataMapTypeIsMutated";
        this._initializedDataKey = "_initializedDataKey";
        this._initializedDataKeyIsMutated = "_initializedDataKeyIsMutated";
        this._isAddToRelationship = "_isAddToRelationship";
        this._isAddToRelationshipIsMutated = "_isAddToRelationshipIsMutated";
        this._jsonStateStr = "_jsonStateStr";
        this._nameIsMutated = "_nameIsMutated";
        this._organizationIsMutated = "_organizationIsMutated";
        this._relationshipPersistableType = "_relationshipPersistableType";
        this.allowImportDataToBeUpdated = "allowImportDataToBeUpdated";
        this.createdDate = "createdDate";
        this.dataMapToken = "dataMapToken";
        this.description = "description";
        this._id = "_id";
        this.importDataMapAttributes = "importDataMapAttributes";
        this.importDataMapType = "importDataMapType";
        this.jsonType = "jsonType";
        this.modifiedDate = "modifiedDate";
        this.name = "name";
        this.organization = "organization";
    }
}
class MetaData {
    constructor() {
        this.namespace = "com.salesstryke.entity.classes.persistable.ImportDataMap";
        this.mongDataModeName = "ImportDataMap";
        this.entityAttributes = new EntityAttributes();
        this.mongoAttributes = new MongoAttributes();
    }
}
class ImportDataMap extends JsonApiPersistableBase_1.JsonApiPersistableBase {
    constructor(p) {
        super(p);
        this._allowImportDataToBeUpdatedIsMutated = false;
        this._dataMapTokenIsMutated = false;
        this._descriptionIsMutated = false;
        this._idSetBySystem = false;
        this._importDataMapAttributesIsMutated = false;
        this._importDataMapTypeIsMutated = false;
        this._initializedDataKey = null;
        this._initializedDataKeyIsMutated = false;
        this._isAddToRelationship = false;
        this._isAddToRelationshipIsMutated = false;
        this._nameIsMutated = false;
        this._organizationIsMutated = false;
        this.allowImportDataToBeUpdated = undefined;
        this.dataMapToken = null;
        this.description = null;
        this.id = null;
        this.importDataMapAttributes = new Array();
        this.importDataMapType = null;
        this.jsonType = "com.salesstryke.entity.classes.persistable.ImportDataMap";
        this.name = null;
        this.organization = null;
    }
    /**
     * Sets all isMutated flags to be false
     * @returns
     */
    _clearIsMutated() {
        this._initializedDataKeyIsMutated = false;
        this._allowImportDataToBeUpdatedIsMutated = false;
        this._dataMapTokenIsMutated = false;
        this._descriptionIsMutated = false;
        this._importDataMapAttributesIsMutated = false;
        this._importDataMapTypeIsMutated = false;
        this._nameIsMutated = false;
        this._organizationIsMutated = false;
        return (this);
    }
    getClassName() {
        return ("com.salesstryke.entity.classes.persistable.ImportDataMap");
    }
    /**
     * MongoRefArray of ImportDataMapAttributes. Use .addImportDataMapAttribute(importDataMap, PersistenceType.ATTACH); to add a MongoRefArray item. Use .removeImportDataMapAttribute(importDataMap, PersistenceType.DETACH); to remove a MongoRefArray item.
     *
     * Default PersistenceType : PersistenceType.ATTACH
     */
    addImportDataMapAttribute(importDataMapAttribute, persistanceType = PersistenceType_1.PersistenceType.ATTACH) {
        if (this._getIsTrackingMutations()) {
            importDataMapAttribute._trackMutations();
        }
        importDataMapAttribute._setRelationshipPersistableType(persistanceType.getId());
        this.importDataMapAttributes.push(importDataMapAttribute);
        return (this);
    }
    /**
     * MongoRefArray of ImportDataMapAttributes. Use .addImportDataMapAttribute(importDataMap, PersistenceType.ATTACH); to add a MongoRefArray item. Use .removeImportDataMapAttribute(importDataMap, PersistenceType.DETACH); to remove a MongoRefArray item.
     * If the Entity ID is null, will not be added.
     * Default PersistenceType : PersistenceType.DETACH
     */
    removeImportDataMapAttribute(importDataMapAttribute, persistanceType = PersistenceType_1.PersistenceType.DETACH) {
        if (this._getIsTrackingMutations()) {
            importDataMapAttribute._trackMutations();
        }
        if (persistanceType === PersistenceType_1.PersistenceType.NONE) {
            let index = this.importDataMapAttributes.findIndex(obj => obj === importDataMapAttribute);
            if (index !== -1) {
                this.importDataMapAttributes.splice(index, 1);
            }
            importDataMapAttribute._setRelationshipPersistableType(PersistenceType_1.PersistenceType.NONE.getId());
            return (this);
        }
        importDataMapAttribute._setRelationshipPersistableType(persistanceType.getId());
        this.importDataMapAttributes.push(importDataMapAttribute);
        return (this);
    }
    /**
     * CREATE, DELETE, UPDATE
     * @returns string | null
     */
    _getEntityState() {
        //return(super._getEntityState());
        return (super._getEntityState());
    }
    /**
     * Is a JSON string that passes the state of attributes from client to server, example: '{'attributes':[{'name':'userName','isMutated':true},{'name':'roles','isMutated':true}]}'
     * @returns string | null
     */
    _getJsonStateStr() {
        //return(super._getEntityState());
        return (super._getJsonStateStr());
    }
    /**
     * A system attribute to identify the relationship persisable type.
     * @returns number
     */
    _getRelationshipPersistableType() {
        //return(super._getEntityState());
        return (super._getRelationshipPersistableType());
    }
    /**
     *
     * @returns Date | null
     */
    getCreatedDate() {
        //return(super._getEntityState());
        return (super.getCreatedDate());
    }
    /**
     * The modified date of the document
     * @returns Date | null
     */
    getModifiedDate() {
        //return(super._getEntityState());
        return (super.getModifiedDate());
    }
    /**
     *
     * @returns boolean
     */
    _getIsAllowImportDataToBeUpdatedMutated() {
        return (this._allowImportDataToBeUpdatedIsMutated);
    }
    /**
     *
     * @returns boolean
     */
    _getIsDataMapTokenMutated() {
        return (this._dataMapTokenIsMutated);
    }
    /**
     *
     * @returns boolean
     */
    _getIsDescriptionMutated() {
        return (this._descriptionIsMutated);
    }
    /**
     * Indicates if the ID was set by the system and that the ID is used to create the documet.
     * @returns boolean
     */
    _getIsIdSetBySystem() {
        return (this._idSetBySystem);
    }
    /**
     *
     * @returns boolean
     */
    _getIsImportDataMapAttributesMutated() {
        return (this._importDataMapAttributesIsMutated);
    }
    /**
     *
     * @returns boolean
     */
    _getIsImportDataMapTypeMutated() {
        return (this._importDataMapTypeIsMutated);
    }
    /**
     * This is a key that is used to initialize data one time. Example: setting the USState information.
     * @returns string | null
     */
    _getInitializedDataKey() {
        return (this._initializedDataKey);
    }
    /**
     *
     * @returns boolean
     */
    _getIsInitializedDataKeyMutated() {
        return (this._initializedDataKeyIsMutated);
    }
    /**
     *
     * @returns boolean
     */
    _getIsAddToRelationship() {
        return (this._isAddToRelationship);
    }
    /**
     *
     * @returns boolean
     */
    _getIsAddToRelationshipMutated() {
        return (this._isAddToRelationshipIsMutated);
    }
    /**
     *
     * @returns boolean
     */
    _getIsNameMutated() {
        return (this._nameIsMutated);
    }
    /**
     *
     * @returns boolean
     */
    _getIsOrganizationMutated() {
        return (this._organizationIsMutated);
    }
    /**
     * Identifies if this is a single importData from a file or if it can be updated with a new data from a new file.
     * @returns boolean
     */
    getAllowImportDataToBeUpdated() {
        return ((this.allowImportDataToBeUpdated === undefined) ? false : this.allowImportDataToBeUpdated);
    }
    /**
     * The description of the datamap.
     * @returns string | null
     */
    getDataMapToken() {
        return (this.dataMapToken);
    }
    /**
     * The description of the datamap.
     * @returns string | null
     */
    getDescription() {
        return (this.description);
    }
    /**
     *
     * @returns string | null
     */
    getId() {
        return (this.id);
    }
    /**
     * MongoRefArray of ImportDataMapAttributes. Use .addImportDataMapAttribute(importDataMap, PersistenceType.ATTACH); to add a MongoRefArray item. Use .removeImportDataMapAttribute(importDataMap, PersistenceType.DETACH); to remove a MongoRefArray item.
     * @returns Array<ImportDataMapAttribute>
     */
    getImportDataMapAttributes() {
        return (this.importDataMapAttributes);
    }
    /**
     * Link to Data Map Type
     * @returns ImportDataMapType | null
     */
    getImportDataMapType() {
        return (this.importDataMapType);
    }
    /**
     *
     * @returns string | null
     */
    getJsonType() {
        return (this.jsonType);
    }
    /**
     * The name of the data map.
     * @returns string | null
     */
    getName() {
        return (this.name);
    }
    /**
     * The organization.
     * @returns Organization | null
     */
    getOrganization() {
        return (this.organization);
    }
    /**
     * CREATE, DELETE, UPDATE
     */
    _setEntityState(_entityState) {
        super._setEntityState(_entityState);
        return (this);
    }
    /**
     * Is a JSON string that passes the state of attributes from client to server, example: '{'attributes':[{'name':'userName','isMutated':true},{'name':'roles','isMutated':true}]}'
     */
    _setJsonStateStr(_jsonStateStr) {
        super._setJsonStateStr(_jsonStateStr);
        return (this);
    }
    /**
     * A system attribute to identify the relationship persisable type.
     */
    _setRelationshipPersistableType(_relationshipPersistableType) {
        super._setRelationshipPersistableType(_relationshipPersistableType);
        return (this);
    }
    /**
     *
     */
    setCreatedDate(createdDate) {
        super.setCreatedDate(createdDate);
        return (this);
    }
    /**
     * The modified date of the document
     */
    setModifiedDate(modifiedDate) {
        super.setModifiedDate(modifiedDate);
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsAllowImportDataToBeUpdatedMutated(_allowImportDataToBeUpdatedIsMutated) {
        this._allowImportDataToBeUpdatedIsMutated = _allowImportDataToBeUpdatedIsMutated;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsDataMapTokenMutated(_dataMapTokenIsMutated) {
        this._dataMapTokenIsMutated = _dataMapTokenIsMutated;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsDescriptionMutated(_descriptionIsMutated) {
        this._descriptionIsMutated = _descriptionIsMutated;
        return (this);
    }
    /**
* ```
     * Indicates if the ID was set by the system and that the ID is used to create the documet.
* ```
     */
    _setIsIdSetBySystem(_idSetBySystem) {
        this._idSetBySystem = _idSetBySystem;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsImportDataMapAttributesMutated(_importDataMapAttributesIsMutated) {
        this._importDataMapAttributesIsMutated = _importDataMapAttributesIsMutated;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsImportDataMapTypeMutated(_importDataMapTypeIsMutated) {
        this._importDataMapTypeIsMutated = _importDataMapTypeIsMutated;
        return (this);
    }
    /**
* ```
     * This is a key that is used to initialize data one time. Example: setting the USState information.
* ```
     */
    _setInitializedDataKey(_initializedDataKey) {
        if (!this._getIsMutateSuppressed() && this._initializedDataKey !== _initializedDataKey) {
            this._setIsInitializedDataKeyMutated(true);
        }
        this._initializedDataKey = _initializedDataKey;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsInitializedDataKeyMutated(_initializedDataKeyIsMutated) {
        this._initializedDataKeyIsMutated = _initializedDataKeyIsMutated;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsAddToRelationship(_isAddToRelationship) {
        if (this._isAddToRelationship !== _isAddToRelationship) {
            this._setIsAddToRelationshipMutated(true);
        }
        this._isAddToRelationship = _isAddToRelationship;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsAddToRelationshipMutated(_isAddToRelationshipIsMutated) {
        this._isAddToRelationshipIsMutated = _isAddToRelationshipIsMutated;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsNameMutated(_nameIsMutated) {
        this._nameIsMutated = _nameIsMutated;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    _setIsOrganizationMutated(_organizationIsMutated) {
        this._organizationIsMutated = _organizationIsMutated;
        return (this);
    }
    /**
* ```
     * Identifies if this is a single importData from a file or if it can be updated with a new data from a new file.
* ```
     */
    setAllowImportDataToBeUpdated(allowImportDataToBeUpdated) {
        if (!this._getIsMutateSuppressed() && this.allowImportDataToBeUpdated !== allowImportDataToBeUpdated) {
            this._setIsAllowImportDataToBeUpdatedMutated(true);
        }
        this.allowImportDataToBeUpdated = allowImportDataToBeUpdated;
        return (this);
    }
    /**
* ```
     * The description of the datamap.
* ```
     */
    setDataMapToken(dataMapToken) {
        if (!this._getIsMutateSuppressed() && this.dataMapToken !== dataMapToken) {
            this._setIsDataMapTokenMutated(true);
        }
        this.dataMapToken = dataMapToken;
        return (this);
    }
    /**
* ```
     * The description of the datamap.
* ```
     */
    setDescription(description) {
        if (!this._getIsMutateSuppressed() && this.description !== description) {
            this._setIsDescriptionMutated(true);
        }
        this.description = description;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    setId(id) {
        this.id = id;
        return (this);
    }
    /**
* ```
     * MongoRefArray of ImportDataMapAttributes. Use .addImportDataMapAttribute(importDataMap, PersistenceType.ATTACH); to add a MongoRefArray item. Use .removeImportDataMapAttribute(importDataMap, PersistenceType.DETACH); to remove a MongoRefArray item.
* ```
     */
    setImportDataMapAttributes(importDataMapAttributes) {
        if (!this._getIsMutateSuppressed() && this.importDataMapAttributes !== importDataMapAttributes) {
            this._setIsImportDataMapAttributesMutated(true);
        }
        this.importDataMapAttributes = importDataMapAttributes;
        return (this);
    }
    /**
* ```
     * Link to Data Map Type
     * persistanceType?: PersistenceType   If the persistanceType is not set and the Entity.id is null or undefined, will be set to PersistenceType.CREATE.
* ```
     */
    setImportDataMapType(importDataMapType, persistanceType) {
        if (!this._getIsMutateSuppressed() && this.importDataMapType !== importDataMapType) {
            PersistableUtils_1.PersistableUtils.mongoRefSetter(importDataMapType, persistanceType);
            this._setIsImportDataMapTypeMutated(true);
        }
        this.importDataMapType = importDataMapType;
        return (this);
    }
    /**
* ```
     *
* ```
     */
    setJsonType(jsonType) {
        this.jsonType = jsonType;
        return (this);
    }
    /**
* ```
     * The name of the data map.
* ```
     */
    setName(name) {
        if (!this._getIsMutateSuppressed() && this.name !== name) {
            this._setIsNameMutated(true);
        }
        this.name = name;
        return (this);
    }
    /**
* ```
     * The organization.
     * persistanceType?: PersistenceType   If the persistanceType is not set and the Entity.id is null or undefined, will be set to PersistenceType.CREATE.
* ```
     */
    setOrganization(organization, persistanceType) {
        if (!this._getIsMutateSuppressed() && this.organization !== organization) {
            PersistableUtils_1.PersistableUtils.mongoRefSetter(organization, persistanceType);
            this._setIsOrganizationMutated(true);
        }
        this.organization = organization;
        return (this);
    }
    _getIsMutated() {
        let isMutated = false;
        this._setCurrentState2({
            attributeName: '_relationshipPersistableType',
            value: this._relationshipPersistableType
        });
        this._setCurrentState2({
            attributeName: 'allowImportDataToBeUpdated',
            value: this.allowImportDataToBeUpdated
        });
        this._setCurrentState2({
            attributeName: 'createdDate',
            value: this.createdDate
        });
        this._setCurrentState2({
            attributeName: 'dataMapToken',
            value: this.dataMapToken
        });
        this._setCurrentState2({
            attributeName: 'description',
            value: this.description
        });
        this._setCurrentState2({
            attributeName: 'id',
            value: this.id
        });
        this._setCurrentState2({
            attributeName: 'importDataMapAttributes',
            value: this.importDataMapAttributes
        });
        this._setCurrentState2({
            attributeName: 'importDataMapType',
            value: this.importDataMapType
        });
        this._setCurrentState2({
            attributeName: 'modifiedDate',
            value: this.modifiedDate
        });
        this._setCurrentState2({
            attributeName: 'name',
            value: this.name
        });
        this._setCurrentState2({
            attributeName: 'organization',
            value: this.organization
        });
        this._currentHashcode2 = this.getHashCode();
        isMutated = super._getIsMutated();
        return (isMutated);
    }
    _trackMutations() {
        if (this._isTrackingingMutations) {
            return (this);
        }
        this._setInitializedState2({
            attributeName: '_relationshipPersistableType',
            value: this._relationshipPersistableType
        });
        this._setInitializedState2({
            attributeName: 'allowImportDataToBeUpdated',
            value: this.allowImportDataToBeUpdated
        });
        this._setInitializedState2({
            attributeName: 'createdDate',
            value: this.createdDate
        });
        this._setInitializedState2({
            attributeName: 'dataMapToken',
            value: this.dataMapToken
        });
        this._setInitializedState2({
            attributeName: 'description',
            value: this.description
        });
        this._setInitializedState2({
            attributeName: 'id',
            value: this.id
        });
        this._setInitializedState2({
            attributeName: 'importDataMapAttributes',
            value: this.importDataMapAttributes
        });
        this._setInitializedState2({
            attributeName: 'importDataMapType',
            value: this.importDataMapType
        });
        this._setInitializedState2({
            attributeName: 'modifiedDate',
            value: this.modifiedDate
        });
        this._setInitializedState2({
            attributeName: 'name',
            value: this.name
        });
        this._setInitializedState2({
            attributeName: 'organization',
            value: this.organization
        });
        super._trackMutations();
        if (!this._hasInitializedHashCode2) {
            this._initializedHashcode2 = this.getHashCode();
        }
        this._hasInitializedHashCode2 = true;
        return (this);
    }
    getHashCode() {
        let hashCode_n = -1;
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.allowImportDataToBeUpdated);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.createdDate);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.dataMapToken);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.description);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.id);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.importDataMapAttributes);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.importDataMapType);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.modifiedDate);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.name);
        hashCode_n += HashCodeUtil_1.HashCodeUtil.getHashCodeFromValue(this.organization);
        return (hashCode_n);
    }
}
exports.ImportDataMap = ImportDataMap;
ImportDataMap.metaDataForImportDataMap = new MetaData();
ClassFor_1.ClassFor.getInstance().addClass("com.salesstryke.entity.classes.persistable.ImportDataMap", ImportDataMap);
//# sourceMappingURL=ImportDataMap.js.map